...
**Hotfix#3 2026-06-04 EDT (restore v70 beam LOOK, same version v80, user "v80 is ugly, make it like v70"):** the hotfix#2 solid hexagon cone was rejected as ugly. `_renderBaseYellowBeam` is now a faithful port of the v70 `drawStandardBeam` beam — a technical near/far-field visualization: translucent near-field caution band (`rgba(210,153,34,0.10)` + dashed edge + bright core triangle from the probe face `bColor 0.70→0.55` + `N=X mm` / `near field` labels) + 3-layer feathered far-field cone (1.25× bloom α0.04 / 1.10× glow α0.10 / main gradient 0.52→0.20→0.05) + **dashed cone-boundary "V" lines from the probe centre** (`bColor 0.45` dash[3,4]) + NF/FF divider + `← far field` + `X MHz · D=Y mm · θ½=Z°` header. Beam runs full to the back wall (no cut/ghost; SH9-a holds). `topHW`/hexagon reverted; `_conePath` reverted to the far-field cone (used only by Stage B's shadow clip) → **shadow below is unchanged** per user "下方陰影不要變動". Contact-shadow column stays disabled for EX1/EX2 (v70 has none). node `--check` clean.

**Hotfix#2 2026-06-04 EDT (beam-emergence, same version v80, user v70 reference screenshots):** ROOT CAUSE of "the beam coming out of the probe disappeared" (`探頭出來的光束不見了`) found by comparing v70 vs v80 screenshots — it was NOT `globalAlpha`. `_conePath` drew the cone starting at `nearFieldY` (~30% deep), leaving the near-field zone (probe face → cone) empty; the dark `rgba(0,0,0)` contact-shadow column then filled that gap, so the beam looked detached from the probe. Two fixes: (1) `_conePath` now draws a true UT beam profile starting at the probe face `beamTop` — wide `topHW = max(ffHWStart+1, TX_W*0.45)` at the probe → necks to `ffHWStart` at the near-field focus → diverges to `hWBot` at the back wall; gradient now spans `beamTop→beamBot` (0.85 at the probe face). (2) the contact-shadow column is now disabled for EX1/EX2 (`resolution`/`penetration`) — it was the element hiding the beam, and the v70 reference has no such column. EX1/EX2 "shadow below" is now provided solely by the Stage B below-defect occlusion (`_applyShadowOcclusion`). No version bump. node `--check` clean.
