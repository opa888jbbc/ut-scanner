...
246\. 【SH9-d · Minimal Residual Shadow】**v78 XS-1-b 已升級 alpha cap 至 0.30~0.50 並改用 createLinearGradient(見 §253)** — 陰影保留為「教學殘留 cue」但**幾乎看不見**:
...
250\. 【SH9-h · Educational Goal】**v78 XS-1-a clarified — 並存模式允許「漸層陰影視覺」共存,因 cone 形狀仍維持完整,陰影不是擋光而是 BW 路徑 visible attenuation cue;blockage paradigm 仍 deprecated(陰影 ≠ cone shape change)**。條文宣示性規則,documentation only:
...
（以下規則 252\~255 由使用者於 2026-06-02 EDT「XS-1 走 B 方案(並存模式)」採納後納入,對應老闆當天追加指令(`uploads/螢幕擷取畫面 2026-06-02 023101.png` 紫色標註截圖)。**Paradigm 並存**:SH9 反射 ripple 範式保留 + XS-1 漸層陰影層獨立繪製;cone 形狀仍維持完整(SH9-a 不違反),陰影是 defect 下方 beam 路徑的「visible attenuation cue」而非擋光物。v78 ship 4 條 XS-1-a..d。)



252\. 【XS-1-a · 並存模式 Paradigm 宣示】SH9 反射 ripple(SH9-b SDH specular 3-packet · SH9-c pore Mie scatter)**全部保留不動**,XS-1 在其上**新增獨立繪製層** — 從 defect 下緣沿 beam 中軸往 BW 方向畫 linear gradient 半透明帶,呈現「被擋住、能量逐漸衰減」陰影感(對齊老闆紫色標註截圖)。
  - **不**改 cone 幾何(SH9-a 保留:cone 永遠從 probe 走到 BW)
  - **不**撤 SH9-b/c 反射視覺(反射主視覺仍主導)
  - **新增**陰影層用 `ctx.createLinearGradient(x0, y0, x1, y1)` 沿 beam 方向繪製,獨立 polygon
  - **scope**:EX01 5 pore + EX02 4 SDH,在 `drawStandardBeam` 內 §247 SH9-e 「下方 polygon × 0.85」之上再疊一層 gradient shadow
...
258\. 【BO-3 · 遮蔽(occlusion)做強 —— 這才是唯一要的效果】使用者要的是「黃色聲波照到東西 → 被遮蔽 → 後方變暗(陰影)」。v79 初版用 ×0.75(全擋殘 0.31)被打回「還是很明顯穿透」。改用 **smoothstep 強遮蔽**:`_occ = smoothstep(interactionBlock)`(高覆蓋→趨近 1)、`_belowFactor = _bloomBelow = 1 − _occ×0.95`。EX01 cluster overlap≈0.92 → _occ≈0.98 → 殘 ~0.07,對比上方 0.82 亮錐 ≈ 11:1 = 黃光在球體後方近乎消失 = 明確遮蔽;部分覆蓋時 smoothstep 平滑(§1 嚴禁突兀切換)。配合 BO-1 粗亮錐,上亮下暗一眼可見。**不加深藍 overlay**(§253 已證偽);遮蔽 = 黃光本身被擋掉,非另疊暗色。
  ...
  - **【2026-06-04 EDT 統一 EX01+EX02 / 使用者紅燈 spec】** 抽成共用函式 `calculateBeamOcclusion(probeX, objLeft, objRight)`:`reach = beamHalfWidthPx() + 物體半寬;raw = max(0, 1 − |probeX−物體中心|/reach);return smoothstep(raw)`。**EX01(getLineDefSignal,孔群)與 EX02(getPlanarSignal,SDH)共用同一函式** → 兩模式遮蔽演算完全一致。`reach` 含物體半寬,自動吃不同形狀 bounds(EX01 寬孔群 vs EX02 點狀 SDH 皆正確)。EX01 改用此 `block`(取代原 clusterCoverage)驅動「球體後方平滑淡出」+ BW;邊緣輕觸時 smoothstep flat-start = 細微暗化、禁止瞬間斷裂。**沿用 EX2 已確認的 smoothstep 曲線(非 `pow(x,1.5)`),以保 EX2 觀感不變且 EX01 完全比照。**
...
261\. 【BO-8 · 光束遮蔽最終版 —— 2026-06-04 EDT 使用者紅燈】**現行 = 還原 `beamGrad` + 黃光自身漸層陰影。** 使用者明示 offscreen / `destination-out` / 像素高斯場「刪了 beamGrad、是額外疊加圖層」→ **全撤**。現行做法:`drawStandardBeam` 保留原黃色錐 polygon + `createLinearGradient` 填充;陰影 = beamGrad **自身 alpha** 在球體下方 1.0→0.0 平滑淡出,漸層軸 `探頭面 → 球體 (interactionX, interactionY)` 傾斜 → 拖動時淡出區**滑動(水暈)**;`calculateBeamOcclusion` = 高斯能量場 `exp(−dist²/2σ²)`(reach 邊緣 renormalise 到 0,中心 1)。**同一 canvas,無 offscreen / destination-out / 藍線 / 額外圖元。** 嚴禁再走 offscreen 像素場路線。以下為**已撤**的 offscreen 原案,僅存 audit:
  ...
  - **一致 + 清場**:EX1 與 EX2 共用同一高斯場 + 同一 `calculateBeamOcclusion`。移除舊 `beamGrad`/bloom/近場帶/錐邊虛線/θ 標籤/NF-FF divider → 畫面**只剩黃色光束 + 球體造成的陰影**;藍線/藍光暈嘗試碼全清。沿用 smoothstep(非 `pow(1.5)`)。
  - smoke:`drawStandardBeam.toString()` 仍含 `interactionBlock` + `_belowFactor` + `beamBot` + `createLinearGradient`;不含 `fadeEndY`/`hWFadeEnd`。ruleCodes 不變(BO-1..BO-4),版本維持 v79(使用者「不用更新版本」)。
...
264\. 【BO-11 · EX3 防禦性 guard + 光束不裁切】`_applyShadowOcclusion` 開頭加 `if (exercise === 'weld') return;` —— EX3 本就走 `drawWeldBeam` 不會呼叫此函式,但依使用者字面指令「在陰影函式加 EX3 強制 return」明確守衛,確保 EX3 永不繪製 beam 陰影。同時宣示:亮錐(Layer A `_renderBaseYellowBeam`)永遠在陰影之前無 clip 繪製,唯一 `ctx.clip()` 包在 save/restore 內僅作用於陰影漸層,**光束渲染路徑不被任何遮罩裁切**(對應使用者「確保渲染路徑不會被任何遮罩裁切」)。
  ...
  - **【2026-06-04 EDT 光束外觀改回 v70 / 使用者「80 版很醜,要跟 70 版一樣」(同 v80,不升版)】** 上一條的「實心六邊形錐 + topHW」被使用者打回「醜」。改為**完整還原 v70 的 beam 視覺**(從 `各版本/ut-scanner-v70.html` `drawStandardBeam` 移植):`_renderBaseYellowBeam` 重寫為 near/far-field 技術視覺化 —— 近場警示帶(半透明黃 `rgba(210,153,34,0.10)` + 虛線邊界 + **由探頭面射出的亮核三角** `bColor 0.70→0.55` + `N=X mm` / `near field` 標籤)+ 遠場三層羽化錐(1.25× bloom α0.04 / 1.10× glow α0.10 / 主漸層 0.52→0.20→0.05)+ **由探頭中心點 (tx,beamTop) 射出的虛線錐邊「V」**(`bColor 0.45` dash [3,4])+ NF/FF 分隔虛線 + `← far field` + `X MHz · D=Y mm · θ½=Z°` 標籤。beam 全程到底牆不裁切(無 cut/ghost,SH9-a)。`topHW`/六邊形撤回;`_conePath` 還原為遠場錐(僅供 Stage B 陰影 clip)→ **下方陰影完全不變,符合使用者「下方陰影不要變動」**。接觸陰影柱仍對 EX1/EX2 停用(v70 無此柱)。
...
非破壞檢測（NDT）與聲波反射原理統整 — 原文已移至 Vault 存底

📎 本段約 600 行 NDT 基礎原理教材原文,已於 2026-06-02 EDT 逐字移至 Vault:
   Obsidian Vault/raw/ndt-principles-原文存底.md
並已編譯為 wiki 原子筆記 + 00_Buddy/100_NDT_物理核心庫.md(中英 glossary 見 wiki/ndt-bilingual-glossary.md)。
查詢 NDT 原理請用 Vault;本 CLAUDE.md 只保留 §1-§255 規則 + 遠端開發規範。
（移出原因:該段與 Vault 重複,精簡專案 spec ~37%;Buddy 控制層 2026-06-02 EDT 整理,原文零損失存於 Vault）
